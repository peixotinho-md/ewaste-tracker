...
/* Indicadores
 *
 * Funções puras: recebem itens, eventos e pontos, devolvem números. Nada de
 * armazenamento nem de DOM, para que os cálculos possam ser conferidos à mão.
 *
 * Moravam em `js/indicadores.js`. Vieram para cá porque são cálculo de
 * DOMÍNIO — dependiam de sete exports deste arquivo e de mais nada, e as duas
 * páginas que os usam já importavam este módulo. Um arquivo a menos, e o
 * import cruzado some junto. */
...
export function calcularIndicadores(itens, eventos, pontos, agora = new Date()) {
  ...
  const pendentes = emCurso
    ...
    .map((i) => ({
      ...
      slaHoras: etapa(i.etapaAtual)?.slaHoras ?? null,
    }))
